...
GET_DATE:
		mov ah, 0x04 ; RTC 바이오스 인터럽트
		int 0x1a

SAVE_ES:
		xor 	bp,	 bp ; 초기화시키기
		mov 	es,	bp
		mov 	ah,	 ch ; ch에는 century의 값이 들어있다.
		call 	LEFT_LOOP
		mov 	ah, ch  
		call 	RIGHT_LOOP
		mov 	ah, cl  ; cl에는 year의 값이 들어있다.
		call 	LEFT_LOOP
		mov 	ah, cl
		call 	RIGHT_LOOP
		mov 	ah, dh  ; dh에는 month의 값이 들어있다.
		call 	LEFT_LOOP
		mov 	ah, dh
		call 	RIGHT_LOOP
		mov 	ah, dl  ; dl에는 day의 값이 들어있다.
		call 	LEFT_LOOP
		mov 	ah, dl
		call 	RIGHT_LOOP
		jmp 	PRINT_DATE
...
LEFT_LOOP:
		shr 	ah, 4   ; 오른쪽으로 4비트 시프트
		add 	ah, 48  ; 아스키 코드값 변환
		mov 	[es:bp], ah ; 저장
		inc		bp
		ret				; call한 주소로 돌아가기
